...
Built on yfinance, an unofficial library against a reverse engineered
Yahoo endpoint, not a published API. It can break without notice if
Yahoo changes anything. Treat failures here as expected, not a bug.
"""
...
# Yahoo requires a cookie/crumb handshake that yfinance's default
# requests-based session doesn't complete, producing the classic
# "Expecting value: line 1 column 1" error, an empty response being
# parsed as JSON. curl_cffi impersonates a real browser's TLS
# fingerprint closely enough to get through it. One shared session,
# reused across tickers and requests rather than rebuilt each time.
